...
You can serve the dist/ folder with any static server. The backend can be deployed separately (e.g., Heroku, Vercel serverless functions) and the ite.config.js proxy removed or reconfigured.
...
- **404 errors on /api/search or /api/stream**: ensure the backend is running and ite.config.js proxy is active. The client must use /api/* paths (not http://localhost:5000/...).
